...
$(function () {
    ...
    editor = KindEditor.create('textarea[id="editor"]', {
        items: [],
        uploadJson: '/seller/upload/file',
        filePostName: 'file'
    });

    new AjaxUpload('#uploadGoodsCoverImg', {
        action: '/seller/upload/file',
        name: 'file',
        autoSubmit: true,
        responseType: "json",
        ...
        success: function (result) {
            if (result.resultCode == 200) {
                ...
                swal({
                    ...
                    window.location.href = "/seller/goods?shopId="+shopId;
                })
            } else {
                $('#goodsModal').modal('hide');
                swal(result.message, {
...
$('#levelOne').on('change', function () {
    $.ajax({
        url: '/seller/categories/listForSelect?categoryId=' + $(this).val(),
        type: 'GET',
        success: function (result) {
            if (result.resultCode == 200) {
                var levelTwoSelect = '';
